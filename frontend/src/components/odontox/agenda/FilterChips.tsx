...
export type AgendaFilter = "all" | number;

type Chip = { id: AgendaFilter; label: string; cor: string | null };

type FilterChipsProps = {
  dentists: Dentista[];
  filter: AgendaFilter;
  onChange: (id: AgendaFilter) => void;
};

// Chips para filtrar a agenda por profissional ('all' = todos).
export default function FilterChips({ dentists, filter, onChange }: FilterChipsProps) {
  const chips: Chip[] = [
    { id: "all" as AgendaFilter, label: "Todos os profissionais", cor: null },
