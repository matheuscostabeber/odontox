import { minutes } from "@/lib/odontox/format";
import { START_H, END_H, HOUR_H } from "@/lib/odontox/constants";
import Avatar from "@/components/odontox/Avatar";
import ConsultaBlock from "@/components/odontox/agenda/ConsultaBlock";
import type { Consulta, Dentista, Paciente } from "@/lib/types";

const hourMarks: { h: number; label: string; top: number }[] = [];
...
type AgendaBoardProps = {
  dentists: Dentista[];
  consultas: Consulta[];
  patientById: (id: number) => Paciente | undefined;
  onConsultaClick: (id: number) => void;
};

// Quadro da agenda: régua de horas + uma coluna por dentista.
export default function AgendaBoard({ dentists, consultas, patientById, onConsultaClick }: AgendaBoardProps) {
  return (
          ...
          return (
            <div key={d.id} style={{ flex: 1, minWidth: 240, borderRight: "1px solid #E6ECEC", background: "#fff" }}>
              <div style={{ position: "sticky", top: 0, zIndex: 2, background: "#fff", borderBottom: "1px solid #E6ECEC", padding: "8px 12px", display: "flex", alignItems: "center", gap: 9, height: 46 }}>
                <Avatar nome={d.nome} color={d.cor} size={30} shape="rounded" fontSize={12} foto={d.fotoUrl} />
