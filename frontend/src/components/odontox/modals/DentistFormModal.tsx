...
export default function DentistFormModal({ entity }: { entity?: Dentista }) {
  ...
  const { form, field, set } = useForm(
    entity
      ? { id: entity.id as number | undefined, nome: entity.nome, cro: entity.cro, especialidade: entity.especialidade, telefone: entity.telefone, email: entity.email, ativo: entity.ativo }
      : { id: undefined as number | undefined, nome: '', cro: '', especialidade: '', telefone: '', email: '', ativo: true }
  );

  const save = () => { saveDentist(form); close(); };

  const toggle = (val: boolean, on: boolean, onColor: string, onBg: string): CSSProperties => ({
